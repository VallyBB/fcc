...
echo "$($PSQL "SELECT name from teams 
        INNER JOIN games ON teams.team_id=games.winner_id WHERE round = 'Eighth-Final' AND year = 2014
        UNION
        SELECT name from teams
        INNER JOIN games ON teams.team_id=games.opponent_id WHERE round = 'Eighth-Final' AND year = 2014
        ORDER BY name")";
